...
//jump
    ...
    void Update()
    {       
        Debug.Log((Vector3.Distance(newPos, legOne.transform.position)));
        if (transform.position.x <= legOne.transform.position.x + 2f){
            
            if ( onFloor == true ||  jumpFlag == true && !Input.GetKey(KeyCode.Q)){
